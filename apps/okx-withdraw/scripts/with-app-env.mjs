...
/**
 * Resolve an npm package's local JavaScript CLI without relying on shell shims.
 *
 * npm places `vite` on POSIX and `vite.cmd` on Windows in node_modules/.bin.
 * `spawn("vite")` does not consult the project's node_modules/.bin when this
 * wrapper is launched directly, and Windows cannot execute the extensionless
 * POSIX shim. Reading the package's declared `bin` entry and launching it with
 * the current Node executable works identically on every platform.
 */
export function resolveLocalPackageBin(command, root = projectRoot()) {
  try {
    const packageFile = join(root, "node_modules", command, "package.json");
    ...
    return join(dirname(packageFile), relativeBin);
  } catch {
    return null;
  }
}
...
function main(argv) {
  ...
  // Receipt Suite apps are intentionally account-free. Keep auth disabled even
  // though the original platform-specific .grok flag file is not copied.
  const env = mergeAppEnv(
    { VITE_AUTH_ENABLED: "false", ...readAppEnv(projectRoot()) },
    process.env,
  );
  const localPackageBin = resolveLocalPackageBin(command);
  const executable = localPackageBin ? process.execPath : command;
  const executableArgs = localPackageBin ? [localPackageBin, ...args] : args;
  const child = spawn(executable, executableArgs, { stdio: "inherit", env });
